...
class DistilledPPO:
    def __init__(self, camera_obs_dim, vector_obs_dim, action_dim, params):
        ...
        self.distill_student = KnowledgeDistillationNetwork(camera_obs_dim, vector_obs_dim, action_dim).to(device)
        ...
        self.optimizer_distill = optim.Adam(self.distill_student.parameters(), lr=params["learning_rate"])
        self.trajectories = ExperienceBuffer(camera_obs_dim, vector_obs_dim, params)
    ...
    def train(self, steps):
        ...
        for _ in range(self.params['train_epochs']):
            generator = self.trajectories.sample(self.mini_batch_size)
            for sample in generator:
                ...
                _, dist_batch, value_ext_batch, value_int_batch = self.get_action(camera_obs, vector_obs)
                ...
                clipped_value_ext = ext_values + torch.clamp(value_ext_batch - ext_values, -self.params["epsilon"], self.params["epsilon"])
                clipped_value_int = int_values + torch.clamp(value_int_batch - int_values, -self.params["epsilon"], self.params["epsilon"])
                v_surr1 = (value_ext_batch - returns).pow(2) + (value_int_batch - returns).pow(2)
                v_surr2 = (clipped_value_ext - returns).pow(2) + (clipped_value_int - returns).pow(2)
                value_loss = 0.5 * torch.max(v_surr1, v_surr2).mean()

                #distillation Training
                self.optimizer_distill.zero_grad()
                teacher_in = self.processor(camera_obs, return_tensors="pt")
                teacher_out = self.distilled_teacher(**teacher_in).last_hidden_state
                ...
                if grad_step % gradient_accumulation_steps == 0:
                    ...
                    nn.utils.clip_grad_norm_(self.distill_teacher.parameters(), 0.5)
