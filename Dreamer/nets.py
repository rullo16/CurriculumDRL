...
class RSSM(nn.Module):
    def __init__(self, deter_dim, stoc_dim, embedding_dim, action_dim, mlp_dim, act_fn=nn.ELU, min_std_dev=1e-4):
        super().__init__()
        self.min_std_dev = min_std_dev
        self.deter_dim, self.stoc_dim, self.embedding_dim = deter_dim, stoc_dim, embedding_dim

        self.fc_input = nn.Sequential(nn.Linear(stoc_dim + action_dim, deter_dim), act_fn())

        self.rnn = nn.GRUCell(deter_dim, deter_dim)

        self.fc_prior = nn.Sequential(nn.Linear(deter_dim, mlp_dim), act_fn(),
                                      nn.Linear(mlp_dim, 2*stoc_dim))
        
        self.fc_posterior = nn.Sequential(nn.Linear(deter_dim + embedding_dim, mlp_dim), act_fn(),
                                          nn.Linear(mlp_dim, 2*stoc_dim))
...
def mlp(in_dim, mlp_dims, out_dim, act_fn=nn.ELU, out_act=nn.Identity):
    if isinstance(mlp_dims, int):raise ValueError("mlp_dims must be a list of integers")
...
class CNNEncoder(nn.Module):
    def __init__(self, obs_shape, num_channels, embedding_dim):
        ...
        if np.prod(output_shape) != embedding_dim:
            ...
            self.fc = nn.Liner(np.prod(output_shape), embedding_dim)
        self.apply(orthogonal_init)

    def _get_output_shape(self, input_shape, layers):
...
class CNNDecoder(nn.Module):
    def __init__(self, input_dim, num_channels, output_shape):
        ...
        C = int(output_shape[0])
        self._fc1 = nn.Linear(input_dim, self.embedding_dim)
        self._decoder = nn.Sequential(
            nn.ConvTranspose2d(self.embedding_dim, 4*num_channels, 5, stride=2), nn.ReLU(),
