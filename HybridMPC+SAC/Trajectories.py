...
class SAC_ExperienceBuffer:
    ...
    def __init__(self, camera_obs_dim, vector_obs_dim, action_dim, params):
        ...
        self.camera_obs = np.zeros((self.buffer_size, *camera_obs_dim), dtype=np.float16)
        self.next_camera_obs = np.zeros((self.buffer_size, *camera_obs_dim), dtype=np.float16)
    ...
    def store(self, camera_obs, vector_obs, action, reward, next_camera_obs, next_vector_obs, done):
        idx = self.ptr
    ...
    def load(self, path):
        with open(path, 'rb') as f:
            # data = pickle.load(f)
            data = np.load(path)
            self.camera_obs = data['camera_obs']
            self.vector_obs = data['vector_obs']
            self.actions = data['actions']
            self.rewards = data['rewards']
            self.next_camera_obs = data['next_camera_obs']
            self.next_vector_obs = data['next_vector_obs']
