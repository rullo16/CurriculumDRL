...
class HybridMPC_SACAgent:
    ...
    def _process_vector(self, vector):
        if not isinstance(vector, torch.Tensor):
            vector = torch.as_tensor(vector, dtype=torch.float16, device=self.device)
        return vector
    ...
    def train(self, trajectories, step):
        ...
        for _ in range(self.params.train_epochs):
            ...
            camera_obs = self._process_image(sample["camera_obs"])
            ...
            next_camera_obs = self._process_image(sample["next_camera_obs"])
            ...
            # -----------------------------------------------------
            replay_buffer = DistillationDataset(trajectories, self.model.convolution_pipeline, num_samples=10000)
            dataloader = DataLoader(replay_buffer, batch_size=64, shuffle=True) 
            for camera_obs, _ in dataloader:
                camera_obs = camera_obs.to(device)
                student_feats = self.model.convolution_pipeline(camera_obs)
